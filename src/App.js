...
import {Card, CardMedia, FormControlLabel, TextField, Switch as MUISwitch} from "@material-ui/core"
...
function BrandHome() {
    ...
    const fetchRandomPosts = () => {
        fetch(`http://35.181.29.44:9000/api/posts/discover?size=100`, {
            method: 'GET',
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
            ...
            setRandomPosts(result.posts)
        })
            .catch(error => {
                console.warn('There was an error while trying to get random posts: ', error)
            })
    }

    const fetchPosts = () => {
        return fetch(`http://35.181.29.44:9000/api/posts/elena?searchquery=${search}&page=${currentPage}&perPage=100&shadowban=false&shadowbanF=true&moodboard=false&moodboardF=true`, {
            method: 'GET',
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
    ...
    const images = (isRandomOrder ? randomPosts : data).map(post => 'https://plaizoriginal.s3.eu-west-3.amazonaws.com/' + post.picture)
    return (
        <div className="App">
            <FormControlLabel
                control={
                    <MUISwitch
                        ...
                        onChange={ev => setIsRandomOrder(ev.target.checked)}
                        name="random"
                        color="primary"
                    />
                }
            ...
            <div style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start'}}>
                {
                    (isRandomOrder ? randomPosts : data).map((post, index) => <Card onClick={() => setSliderIndex(index)} key={post.id || post._id} style={{width: 300, margin: 20,flexBasis: '20%'}} elevation={24}>
                        <CardMedia
